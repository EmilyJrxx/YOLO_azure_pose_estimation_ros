...
class aubo_vision_pick(object):
    ...
    def put_down(self):
        ...
        # define pose with specific parameters
        pose_goal = current_pose
        pose_goal.position.z -= 0.1 # put_down
    ...
    def goto_next(self, scale):
        ...
        # define pose with specific parameters
        pose_goal = current_pose
        pose_goal.position.y += scale * 0.08 # next
    ...
    def write_R(self, scale):
        ...
        # the second stroke -|-\
        wpose.position.z -= 0.03
        wpose.position.x -= scale * 0.1   # go back
        waypoints.append(copy.deepcopy(wpose))
...
if __name__=="__main__":
    ...
    scale = 0.5
    print "==== Press `Enter` to write R ===="
    # raw_input() is one way you can test your patterns respectively
    aubo_move.write_R(scale)
    print "==== Press `Enter` to write F ===="
    # raw_input()
    aubo_move.goto_next(scale)
    aubo_move.write_F(scale)

    print "==== Press `Enter` to write X ===="
    # raw_input()
    aubo_move.goto_next(scale)
    aubo_move.write_X(scale)

    print "==== Press `Enter` to write S ===="
    # raw_input()
    aubo_move.goto_next(scale)
    aubo_move.write_S(scale)

    print "==== Press `Enter` to write B ===="
    # raw_input()
    aubo_move.goto_next(scale)
    aubo_move.write_B(scale)
